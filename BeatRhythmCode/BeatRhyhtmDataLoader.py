class BeatRhyhtmDataLoader:
    ...
    --------------------
    Loader for pairing WAV and MIDI files from separate folders,
    and saving them into a JSON cache for faster scanning.

    Features:
      - Automatically mounts Google Drive (if not mounted)
      - Scans WAV and MIDI folders
      - Matches files with the same base name
      - Saves results to JSON cache
      - Provides file validation and summary utilities
    """
    ...
    def _load_and_match_files(self):
        """Scan WAV & MIDI folders, match files, and save results to cache."""
        ...
        # Scan folders if cache doesn’t exist
        if self.verbose:
            print(f"\n📂 Scanning folders:\n- WAV : {self.wav_dir}\n- MIDI: {self.midi_dir}")
        ...
        # Match WAV and MIDI files by name
        for wav in wavs:
    ...
    def get_pairs(self):
        """Return list of matched (WAV, MIDI) file pairs."""
        return self.pairs

    def validate_files(self):
        """Validate the existence of all WAV and MIDI files."""
        if not self.verbose:
            return
        print("\n🔍 Validating WAV & MIDI files:")
    ...
    def summary(self):
        """Display a short summary of total data and folder paths."""
        if not self.verbose:
            return
        print("\n📊 Data Summary:")
